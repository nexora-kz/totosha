...
function Card({ icon, title, text, tags }: { icon: React.ReactNode; title: string; text: string; tags?: string[] }) {
  return (
    <article className="card">
      ...
      <h3>{title}</h3>
      <p>{text}</p>
      {tags && <div className="feature-list">{tags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div>}
    </article>
  );
}
    ...
    <header className="header">
      <div className="container">
        <div className="nav">
          ...
          <nav className="links" aria-label="Основная навигация">{nav.map(([href, label]) => <a href={href} key={href}>{label}</a>)}</nav>
          <div className="actions"><a className="btn btn-light" href="/parents">Родителям</a><a className="btn btn-dark" href="/contacts">Записаться</a></div>
          <button className="mobile-btn" type="button" aria-label={open ? 'Закрыть меню' : 'Открыть меню'} onClick={() => setOpen((value) => !value)}>{open ? <X /> : <Menu />}</button>
        </div>
        <div className={`mobile-menu ${open ? 'open' : ''}`}><a href="/">Главная</a>{nav.map(([href, label]) => <a href={href} key={href}>{label}</a>)}</div>
      </div>
    </header>
  );
}
    ...
    <section className="hero">
      ...
      <div className="container hero-grid">
        <div>
          ...
          <p className="lead">Безопасность, развитие, понятный режим и открытая связь с семьёй. Мы не обещаем лишнего — показываем, как устроен детский сад сегодня и что развивается поэтапно.</p>
          <div className="hero-actions">
            <a className="btn btn-primary" href={whatsappUrl('Здравствуйте. Хочу записаться на экскурсию в Тотоша.')} onClick={() => track('cta_whatsapp', { location: 'hero', intent: 'excursion' })}>Записаться на экскурсию <ChevronRight size={20} /></a>
            <a className="btn btn-light" href="#family-path">Как проходит знакомство</a>
          </div>
          <div className={styles.trustStrip}>{facts.map(([title, text]) => <div className={styles.trustItem} key={title}><strong>{title}</strong><span>{text}</span></div>)}</div>
        </div>
        <div className={styles.honestPanel}>
          ...
          ].map(([label, status]) => <div className={styles.statusRow} key={label}><span>{label}</span><span className={styles.status}>{status}</span></div>)}
        </div>
      </div>
    </section>
  );
...
function Day() {
  const rows = [
    ['07:30', 'Приём детей'], ['08:30', 'Завтрак'], ['09:00', 'Развивающие занятия'], ['10:30', 'Прогулка'], ['12:00', 'Обед'], ['13:00', 'Отдых'], ['15:30', 'Полдник'], ['16:00', 'Дополнительные занятия'], ['17:30', 'Игры и свободная деятельность'], ['19:00', 'Завершение дня'],
  ];
...
function LeadForm() {
  ...
  return <div className="form">
    ...
    <label className={styles.formLabel} htmlFor="lead-name">Ваше имя</label><input id="lead-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Как к вам обращаться" />
    <label className={styles.formLabel} htmlFor="lead-phone">Телефон</label><div className="phone-wrap"><span className="phone-prefix">+7</span><input id="lead-phone" type="tel" inputMode="numeric" maxLength={10} value={digits} onChange={(event) => setDigits(event.target.value.replace(/\D/g, '').slice(0, 10))} placeholder="7071230108" /></div><div className="phone-help">Введите 10 цифр после +7.</div>
    <label className={styles.formLabel} htmlFor="lead-intent">Что вас интересует</label><select id="lead-intent" value={intent} onChange={(event) => setIntent(event.target.value as (typeof intentOptions)[number])}>{intentOptions.map((option) => <option key={option}>{option}</option>)}</select>
    <label className={styles.formLabel} htmlFor="lead-comment">Комментарий</label><textarea id="lead-comment" value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Возраст ребёнка или удобное время для звонка" rows={4} />
    <label className={styles.consent}><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span>Согласен на обработку контактных данных для ответа на обращение. <a href="/privacy">Подробнее</a>.</span></label>
    <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', marginTop: 14 }} type="button" onClick={submit} disabled={sending}><Send size={18} /> {sending ? 'Отправляем...' : 'Отправить заявку'}</button>
    {error && <div className={styles.fallback}>{error}<div className={styles.successActions}><a className="btn btn-light" href={whatsappUrl(message)}>Написать в WhatsApp</a><a className="btn btn-light" href={TOTOSHA_CONTACTS.telUrl}>Позвонить</a></div></div>}
    {sent && <div className="success"><b>Заявка отправлена</b><p>Заведующая свяжется с вами для уточнения возраста ребёнка и удобного времени экскурсии.</p><div className={styles.successActions}><a className="btn btn-light" href={whatsappUrl(message)}>Дополнить в WhatsApp</a><a className="btn btn-light" href={TOTOSHA_CONTACTS.telUrl}>Позвонить сейчас</a></div></div>}
  </div>;
}
